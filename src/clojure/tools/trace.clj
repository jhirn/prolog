...
(defn ^{:private false} trace-indent
...
symbol name of the function."
  ...
  (let [id (gensym "t")]
    ...
    (let [value (binding [*trace-depth* (inc *trace-depth*)]
                  (apply f args))]
      ; modified by sean neilan
      (tracer *trace-depth* (str (trace-indent) "=> " (pr-str value))) ; was *trace-depth* instead of id

      ;(tracer id (str (trace-indent) "=> " (pr-str value))) ; was *trace-depth* instead of id
...
(defn ^{:private false} trace-bindings
